...
const DEFAULT_EFFECT_OPTIONS = {
  ...
  // Kadosh Tech palette: navy road/background, brand-blue traffic and
  // sticks, white lane markings — no hues outside the existing site tokens.
  colors: {
    roadColor: 0x0b1220,
...
const Hyperspeed = forwardRef(({ effectOptions = DEFAULT_EFFECT_OPTIONS, onReady }, ref) => {
  const hyperspeed = useRef(null);
  const appRef = useRef(null);

  // Lets a scroll-driven timeline (see Hero.jsx) push values into the
  // running scene without re-rendering/recreating it: App#update(delta)
  // reads these two fields every frame and lerps toward them itself.
  useImperativeHandle(ref, () => ({
    setSpeedUpTarget: (value) => {
      if (appRef.current) appRef.current.speedUpTarget = value;
    },
    setFovTarget: (value) => {
      if (appRef.current) appRef.current.fovTarget = value;
    },
  }), []);
  ...
  useEffect(() => {
    ...
    myApp.loadAssets().then(() => {
      // StrictMode's dev double-invoke disposes the previous instance
      // synchronously, but doesn't cancel this in-flight promise — if a
      // newer instance already replaced `myApp` by the time this resolves,
      // initializing on the disposed one throws deep inside postprocessing
      // (its WebGL context/render targets are already torn down).
      if (appRef.current !== myApp) return;
      myApp.init();
      onReady?.();
    });
